...
MSRV_IRTOUCH*MSRV_IRTOUCH::m_pInstance=NULL;

MSRV_IRTOUCH::MSRV_IRTOUCH()
{
...
MSRV_IRTOUCH *MSRV_IRTOUCH::GetInstance()
{
    if(m_pInstance ==NULL)
    {
        m_pInstance = new MSRV_IRTOUCH ();
        assert(m_pInstance);
    }
    return m_pInstance;
}

void MSRV_IRTOUCH::DestoryInstance()
{
    if(m_pInstance !=NULL)
    {
        delete m_pInstance;
...
void MSRV_IRTOUCH::signal_handler(int sig)
{
    if (sig == SIGINT)
    {
            HHT_LOG_DEBUG("============ctrl+c to terminate.============\n");
            MSRV_IRTOUCH::GetInstance()->uart_close(uart_tty_fd);
            uart_tty_fd =-1;
            MSRV_IRTOUCH::GetInstance()->hid_device_close(usb_hid_fd);
            usb_hid_fd = -1;
            MSRV_IRTOUCH::GetInstance()->sleep_ms(1000);
            exit(0);
    }
}
char *MSRV_IRTOUCH::convert_hex_to_str(uint8_t *pBuf, const int nLen,const bool isHex)
{
...
u8 MSRV_IRTOUCH::DoCalibration(u16 *xvalue, u16 *yvalue)
{
    u16 X,Y;
    s64 X_X,Y_Y;
...
int MSRV_IRTOUCH::create_virtual_input_device(void)
{
...
#if  1
    strncpy(uinp.name,"IR-SERIAL-TOUCH TOUCHSCREEN",sizeof(uinp.name)-1);
...
void *MSRV_IRTOUCH::Run(void *arg)
{
    MSRV_IRTOUCH *msrv = (MSRV_IRTOUCH*)arg;
    uint8_t m_rvbuf[64]={0};
    int usb_hid =-1;
    usb_hid = msrv->hid_device_open();
    if(usb_hid<0)
    {
        HHT_LOG_ERROR("===>operate on device:[%s]\n open istuch hid drv device failed [%d].\n\n", HID_TOUCH_DEVICE,usb_hid);
        return ((void*)0);
    }
    else
    {
        HHT_LOG_DEBUG("===>operate on device:[%s]\n open irtouch hid drv device status:[%d]\n\n", HID_TOUCH_DEVICE,usb_hid);
    }
    //写入指令
    #if 1
        msrv->hid_device_write(usb_hid_fd, ENABLE_HID_RAW_DATA);
        HHT_LOG_DEBUG("send_usb_hid_enable_command:[%s]\n", ENABLE_HID_RAW_DATA);
    #else
        msrv->hid_device_write(usb_hid_fd, DISABLE_HID_RAW_DATA);
        HHT_LOG_DEBUG("send_usb_hid__disable_command:[%s]\n", DISABLE_HID_RAW_DATA);
    #endif
    msrv->sleep_ms(3000);
    ...
    while (1)
    {
       ...
       {
        #if 1
            HHT_LOG_DEBUG("read data from irtouch hid drv: \n");
            ...
            HHT_LOG_DEBUG("read data from irtouch hid size:[%d] \n",buflen);
        #endif
            if ((m_rvbuf[0] == 0xfc) && (m_rvbuf[1] == 0xfe) && (m_rvbuf[2] == 0x81))//enable / disable commands
            {
                HHT_LOG_DEBUG("reply(Success): IRTouch --> Android: [%s]",msrv->convert_hex_to_str(m_rvbuf,sizeof(m_rvbuf),false));
            }
            else
            {
                //处理接收到的合法数据
       ...
       {
          HHT_LOG_ERROR("irtouch incoming data len: [%d]\n",buflen);
          memset(m_rvbuf,0,sizeof(m_rvbuf));
       }
        msrv->sleep_ms(1);
    }
...
int MSRV_IRTOUCH::hid_device_open()
{
...
void MSRV_IRTOUCH::hid_device_close(int hid_device)
{
    if (hid_device >0)
    {
        close(hid_device);
...
int MSRV_IRTOUCH::hid_device_write(int device_fd,uint8_t *cmd)
{
    int size = write(device_fd,cmd,sizeof(cmd));
    return size;
}

int MSRV_IRTOUCH::hid_device_read(int device_fd,uint8_t *rvbuf)
{
    int size = read(device_fd,rvbuf,sizeof(rvbuf));
    return size;
}

int MSRV_IRTOUCH::check_incoming_data(uint8_t *rvbuf, int len)
{
    //校验从HID接收的一帧数据
...
void MSRV_IRTOUCH::handle_incoming_data(uint8_t *rvbuf, int len)
{
    ...
    for (int i = 0; i < g_points; i++)
    {
        ...
        if(m_info->m_touch_points[i].touch_status==STATUS_DOWN)
        {
            m_data->un_data.touch.item[i].status =0x01;//down
            HHT_LOG_DEBUG("====> ststus[0x%04x]-down\n",m_info->m_touch_points[i].touch_status);
        }
        else if (m_info->m_touch_points[i].touch_status==STATUS_UP)
        {
            m_data->un_data.touch.item[i].status = 0x00;//up
            HHT_LOG_DEBUG("====> ststus[0x%04x]-up\n", m_info->m_touch_points[i].touch_status);
        }
        else 
        {
            m_data->un_data.touch.item[i].status =0x00;
            HHT_LOG_DEBUG("====> ststus[0x%04x] others\n",m_info->m_touch_points[i].touch_status);
        }
...
Report_touch_info *MSRV_IRTOUCH::get_report_info(uint8_t *rvbuf, int len)
{
    //rvbuf为64bytes完整一帧数据
...
void MSRV_IRTOUCH::write_sndbuf_to_uart(const char *Byte, int num) 
{
    if (uart_tty_fd >= 0)
    {
...
void MSRV_IRTOUCH::report_info_to_android(Touch_point_info *info,bool *touch_down_flag)//上报触摸数据至Android
{
    if(info !=NULL)
    {
...
bool iseven = true;
bool MSRV_IRTOUCH::trans_hht_touch_w_h_pos(trans_point_data frame)
{
#if 1
...
void MSRV_IRTOUCH::transfer_report_info_to_uart(Report_touch_info *info)
{
    write(uart_tty_fd,info,sizeof(Report_touch_info));
}


void MSRV_IRTOUCH::sleep_ms(unsigned int msec)
{
    struct timeval tval;
...
int MSRV_IRTOUCH::m_strlen(uint8_t *s)
{
    int i=0;
    while(*s!='\0')
    {
...
void MSRV_IRTOUCH::show_termios(const termios *s)
{
    if( s )
    {
#if 0
...
int MSRV_IRTOUCH::get_termios(int fd, termios *s)
{
    if( -1 == fd || 0 == s )
    {
        return -1;
...
int MSRV_IRTOUCH::set_termios(int fd, const termios *s)
{
    if( -1 == fd || 0 == s )
    {
        return -1;
...
int MSRV_IRTOUCH::baud_to_speed(int baud)
{
    switch( baud )
    {
    case B1200:
...
int MSRV_IRTOUCH::get_speed(const termios *s)
{
    if( s )
    {
...
int MSRV_IRTOUCH::get_ispeed(const termios *s)
{
    if( s )
    {
...
int MSRV_IRTOUCH::get_ospeed(const termios *s)
{
    if( s )
    {
...
int MSRV_IRTOUCH::get_bsize(const termios *s)
{
    if( s )
    {
...
char MSRV_IRTOUCH::get_parity(const termios *s)
{
    if( s )
    {
        if( s->c_cflag & PARENB )
...
int MSRV_IRTOUCH::get_stop(const termios *s)
{
    if( s )
    {
        if( s->c_cflag & CSTOPB )
...
int MSRV_IRTOUCH::speed_to_baud(int speed)
{
    switch(speed)
    {
    case 1200:
...
int MSRV_IRTOUCH::set_speed(termios *s, int speed)
{
    if( s )
    {
        s->c_cflag &= ~CBAUD;
...
int MSRV_IRTOUCH::set_ispeed(termios *s, int speed)
{
    if( s )
    {
...
int MSRV_IRTOUCH::set_ospeed(termios *s, int speed)
{
    if( s )
    {
...
int MSRV_IRTOUCH::set_bsize(termios *s, int bsize)
{
    if( s )
    {
        s->c_cflag &= ~CSIZE;
...
int MSRV_IRTOUCH::set_parity(termios *s, char parity)
{
    if( s )
    {
        switch(parity)
...
int MSRV_IRTOUCH::set_stop(termios *s, int stop)
{
    if( s )
    {
        if( 1 == stop )
...
int MSRV_IRTOUCH::enable_read(termios *s)
{
    if( s )
    {
        s->c_cflag |= CREAD;
...
int MSRV_IRTOUCH::disable_read(termios *s)
{
    if( s )
    {
        s->c_cflag &= ~CREAD;
...
int MSRV_IRTOUCH::enable_flow_control(termios *s)
{
    if( s )
    {
        s->c_cflag |= CRTSCTS;
...
int MSRV_IRTOUCH::disable_flow_control(termios *s)
{
    if( s )
    {
        s->c_cflag &= ~CRTSCTS;
...
int MSRV_IRTOUCH::uart_open(const char *path)
{
    if( path )
    {
...
int MSRV_IRTOUCH::uart_config(int fd, int baud, char parity, int bsize, int stop)
{
    if( fd < 0 )
    {
        return -1;
...
int MSRV_IRTOUCH::uart_read(int fd, char *buf, int len)
{
    if( fd < 0 || 0 == buf || len < 1 )
    {
        return -1;
...
int MSRV_IRTOUCH::uart_write(int fd, const char *data, int len)
{
...
int MSRV_IRTOUCH::uart_close(int fd)
{
    return close(fd);
}

void MSRV_IRTOUCH::send_mt_abs_touch_key_down_event(int pos_id, int xpos, int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_key_up_event(int pos_id, int xpos, int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_figner_down_event(int pos_id, int xpos, int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_figner_up_event(int pos_id, int xpos, int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_pen_down_event(int pos_id, int xpos, int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_pen_up_event(int pos_id,int xpos,int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_stylus_down_event(int pos_id,int xpos,int ypos)//STYLUS
{
	struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_stylus_up_event(int pos_id,int xpos,int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_rubber_down_event(int pos_id, int xpos, int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_touch_rubber_up_event(int pos_id,int xpos,int ypos)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
void MSRV_IRTOUCH::send_mt_abs_event(int pos_id, int abs_x, int abs_y)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    gettimeofday(&event.time, NULL);
...
int MSRV_IRTOUCH::report_key_event(int fd, unsigned short code, int pressed, timeval *time)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
...
int MSRV_IRTOUCH::report_rel_event(int fd, unsigned short code, int value, timeval *time)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    event.time.tv_sec=time->tv_sec;
...
int MSRV_IRTOUCH::report_abs_event(int fd, unsigned short code, int value, timeval *time)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    event.time.tv_sec=time->tv_sec;
...
int MSRV_IRTOUCH::report_sync_event(int fd, int code, timeval *time)
{
    struct input_event event;
    memset(&event, 0, sizeof(event));
    event.time.tv_sec=time->tv_sec;
